...
class Basic(commands.Cog):
    ...
    async def check_craft(self, ctx: commands.Context):
        ...
        if len(error_blacklist) > 0:
            embed.add_field(name="금지 부품 사용됨", value=f"🟥 {', '.join(error_blacklist)}")
            berror_blacklist = True
        else:
            embed.add_field(name="금지 부품 미사용", value="_🟩_")
        if len(error_armorthickness) > 0:
            ...
            embed.add_field(name="장갑 두께 변경됨", value=f"🟥 {s}")
            berror_armorthickness = True
        else:
            embed.add_field(name="장갑 두께 정상", value="_🟩_")
        ...
        if len(error_tweak) > 0:
            ...
            for k, v in error_tweak:
                ...
                if v[1] == "u":
                    ...
                    s = s + " DT"
            embed.add_field(name="트윅스케일 위반", value=f"🟥 {s}")
            berror_tweak = True
        else:
            embed.add_field(name="트윅스케일 정상", value="_🟩_")
